...
-- Row-Level Security (Oracle VPD) sobre RESERVA: cada conexion solo ve
-- las filas que le corresponden, aplicado a nivel de BASE DE DATOS, no
-- solo en el codigo del backend - asi, aunque el backend tuviera un bug
-- de autorizacion (como los que ya se corrigieron en el codigo), la
-- base de datos igual no entrega filas ajenas a `turismouq_app`.
--
-- Alcance: solo la tabla RESERVA (la que tiene id_cliente directo).
-- RESERVA_HABITACION y PAGO se protegen por el filtro de aplicacion en
-- el codigo (ya corregido), no por RLS, para no aumentar demasiado el
-- riesgo/alcance de esta capa extra.
--
-- IMPORTANTE: el usuario `turismouq` (dueno del esquema, el que usas
-- para las Entregas 1/2/3) queda EXENTO de esta politica gracias al
-- GRANT EXEMPT ACCESS POLICY del script anterior - tus consultas y
-- demos de la base de datos academica no se ven afectadas en nada.
--
...
-- ---------------------------------------------------------------------
-- Contexto de aplicacion: solo se puede modificar a traves del paquete
...
CREATE OR REPLACE PACKAGE pkg_contexto_app AS
  -- Se llama UNA vez al abrir la conexion para cada peticion HTTP,
  ...
  -- actua "como" un cliente especifico, sino con la confianza que da
...
-- ---------------------------------------------------------------------
-- Funcion de politica: por defecto (sin contexto reconocido) NO deja
...
BEGIN
  ...
  ELSE
    RETURN '1 = 0'; -- ningun contexto reconocido -> no se ve ninguna fila
  END IF;
...
-- corre con los privilegios de su DUENO (turismouq, exento del RLS), asi
-- que crear una reserva nunca se ve afectado por esta politica.
